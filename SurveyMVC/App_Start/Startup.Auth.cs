...
namespace SurveyMVC
{
    public partial class Startup
    {
        ...
        public void ConfigureAuth(IAppBuilder app)
        {
            ...
            InitiateRolesAndAdmin();
        }

        private void InitiateRolesAndAdmin()
        {
            ...
            if (!roleManager.RoleExists(Role.Admin.ToString()))
            {
                ...
                adminRole.Name = Role.Admin.ToString();
                roleManager.Create(adminRole);

                var admin = new ApplicationUser();
                admin.UserName = "admin";
                string password = "admin2468";
                if (userManager.Create(admin, password).Succeeded)
                {
                    userManager.AddToRole(admin.Id, Role.Admin.ToString());
                }
            }
            if (!roleManager.RoleExists(Role.Employee.ToString()))
            {
                var employeeRole = new IdentityRole();
                employeeRole.Name = Role.Employee.ToString();
